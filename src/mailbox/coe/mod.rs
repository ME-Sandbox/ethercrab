    ...
    S: Deref<Target = SubDevice>,
{
    ...
    {
        let (read_mailbox, write_mailbox) = crate::mailbox::wait_for_mailboxes(self.subdevice)
            .await
            .inspect_err(|err| {
                fmt::error!(
                    "{} {} {}",
        ...
        let mut response =
            crate::mailbox::wait_for_mailbox_response(self.subdevice, &read_mailbox).await?;
    ...
    async fn send_sdo_info_service(
        ...
        let (read_mailbox, write_mailbox) =
            match crate::mailbox::wait_for_mailboxes(self.subdevice).await {
