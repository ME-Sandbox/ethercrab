...
- Ethernet over EtherCAT (EoE) wire types: `EoeHeader`, `FrameType`, `EoeResult`,
  ...
  SubDevice port. It differs from the reference implementation in one place on purpose: a
...
- [#365](https://github.com/ethercrab-rs/ethercrab/pull/365) (@Dirreke) Add `io-uring` feature so
  ...
  ```toml
  ethercrab = { version = "0.8.0", default-features = false, features = [ "std" ] }
  ```
...
- [#274](https://github.com/ethercrab-rs/ethercrab/pull/274) (@1cedsoda) Added
  `SubDeviceRef::sdo_read_array` to read all sub-indices from a given SDO.
